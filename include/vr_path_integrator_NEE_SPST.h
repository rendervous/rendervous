bool sample_collision_position(map_object,
    ...
    while (true) {
        float md;
        float m = majorant(object, xc, w, md); // to avoid creating outliers with s / m
...
//    USING_SEED(before_direct_environment, environment_bw(object, w, dL_ddirectenv));
    ...
    while (d > EPS)
    {
        ...
        if (mis_allowed) {
            vec3 x_T = mix(prev_x, x, random());
            float weight_T = length(x - prev_x);
            float sigma_T = sigma(object, x_T);
            float powered_C = pow(sigma_C, 3);
            float powered_T = pow(sigma_T, 3);
            float mis_C = powered_C / (1 + powered_C);
            float mis_T = weight_T / (1 + powered_T);

            float mis_W = mis_C + mis_T;
            if (random() * mis_W < mis_T) // transmittance
            {
                x = x_T; // move intersection back to singular
                mis_allowed = false;
                W_singular = sigma_T;
            }
    ...
    while (d > EPS)
    {
        ...
        float dL_dtau = -k;
        float dL_dsigma = is_singular ? dot(dL_dR, Rf) : k / max(0.0001, sigma_C); // to avoid outliers
        // R = <T(x, xc)> * further_R
        ...
        SWITCH_SEED(main_seed, secondary_seed);
        st_tau_bw(object, x, prev_x, max(1, samples), dL_dtau);
        samples = 0;
