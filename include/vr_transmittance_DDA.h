float transmittance_DDA(map_object, vec3 x, vec3 w)
{
    float tMin, tMax;
    ...
    while(current_t < d - 0.0001){
        ...
        tau += sigma_value * (next_t - current_t);
...
void transmittance_DDA_bw(map_object, vec3 x, vec3 w, float dL_dT)
{
    if (dL_dT == 0)
    return;
    ...
    while(current_t < d - 0.0001){
        ...
        float dL_dsigma = dL_dtau * (next_t - current_t);
